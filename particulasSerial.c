...
int main() {
    ...
    printf("Simulacao iniciada (SERIAL):\n\n");
...
void atualizaPosicaoParticulas(struct Particula particulas[]){
    ...
    for(i=0; i<N; i++){
        separacao = regraSeparacao(i, particulas);
        alinhamento = regraAlinhamento(i, particulas);
        coesao = regraCoesao(i, particulas);

        if(particulas[i].infectada == 1)
            regraContagio(i, particulas);

        //aplica pesos
        separacao = mult(separacao, 1.5);
        alinhamento = mult(alinhamento, 1);
        coesao = mult(coesao, 1);

        //aplica forças
        particulas[i].aceleracao = soma(particulas[i].aceleracao, separacao);
        particulas[i].aceleracao = soma(particulas[i].aceleracao, alinhamento);
        particulas[i].aceleracao = soma(particulas[i].aceleracao, coesao);
